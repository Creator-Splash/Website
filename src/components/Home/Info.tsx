...
function InfoBlock({
    ...
    let underlineAlign = "mx-auto sm:mx-auto md:mx-auto";
    if (reverse) underlineAlign = "mx-auto sm:mx-auto md:mx-auto lg:ml-auto";
    else underlineAlign = "mx-auto sm:mx-auto md:mx-auto lg:ml-0";
    // On small screens, always center
    const underlineClasses = "block h-1 w-20 bg-white my-2";
    ...
    return (
      ...
      `}
        >
            <div className="max-w-[600px] w-full px-4 sm:px-6 md:px-8 lg:px-0">
                <Image
                    src={image}
                    alt={title}
                    width={600}
            ...
            <div className="max-w-[600px] w-full px-4 sm:px-6 md:px-8 lg:px-8 mt-8 lg:mt-0">
                <h1
                    className={`
            text-3xl sm:text-4xl md:text-4xl lg:text-5xl font-extrabold uppercase mb-2 text-white font-sans
          `}
                >
                    {title}
                </h1>
                ...
                {paragraphs.map((p, i) => (
                    <p
                        key={i}
                        className="text-sm sm:text-base md:text-base lg:text-lg leading-relaxed mt-5 font-sans text-white"
                    >
                        {p}
                    </p>
                ))}
                {link && (
                    <a
                        href={link.href}
                        className="block font-extrabold text-sm sm:text-base md:text-base lg:text-lg mt-5 text-white hover:underline"
                    >
                        {link.text}
                    </a>
                )}
...
export default function Informations() {
    return (
        <section
            className="
        flex flex-col gap-8 mt-[50px] mb-[30px]
        font-sans py-4
        sm:gap-12 sm:mt-[60px] sm:mb-[40px] sm:py-6
        md:gap-16 md:mt-[80px] md:py-8
        lg:gap-[80px] lg:mt-[100px] lg:mb-[50px]
